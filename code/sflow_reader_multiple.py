...
def process_metadata(metadata,year_marker = 'Station Number', lat_lon_marker ='Latitude'):
    ...
    year = int(metadata[index_year].split('Year:')[1].split(',')[0])
    index_lat_lon = np.where(np.char.startswith(metadata, lat_lon_marker))[0][0]
    lat = metadata[index_lat_lon].split('Latitude :')[1].split(',')[0].strip()
    lat = convert_deg_to_decimal(lat)
    lon = metadata[index_lat_lon].split('Longitude :')[1].split(',')[0].strip()
...
def process_block(data_in, block_start, block_stop, filename):
    ...
    #LATEST IDEA!!!
    df= pd.read_csv(filename,skiprows=global_start,nrows=num_rows,skip_blank_lines=True )
...
for n in name:
    ...
    header = []
    # We open the whole file to be able to find where the blocks of data are
    with open(file) as f:
...
#Sort values, here we did it by time, then lat, then lon
dataf.sort_values(by=['time','lat','lon'],inplace=True)
...
#plt.show()
#plt.clf()
xrst.to_netcdf(path+'files/out_flow_mult.nc',mode='w')
#We can also output our dataframe in this new
#organisation to a csv file (other formats available)
data_topd.to_csv(path+'files/out_flow_mult.csv')
